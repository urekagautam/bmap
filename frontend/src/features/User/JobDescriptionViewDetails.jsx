...
export default function JobDescriptionViewDetails() {
    ...
    try {
      ...
      if (daysDiff < 0) {
        ...
        if (weeksDiff === 1) {
          ...
          return `Apply before ${monthsDiff} month${monthsDiff > 1 ? "s" : ""} from now`
        }
      } else {
  ...
  const formatExperienceRequired = () => {
    ...
    } else if (vacancyData.experienceCriteria) {
      experienceText = formatExperienceCriteria(vacancyData.experienceCriteria)
    } else if (vacancyData.experience) {
      experienceText = vacancyData.experience
    } else {
